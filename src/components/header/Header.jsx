...
export default function Header() {
  ...
  useEffect(() => {
    // const data = JSON.parse(localStorage.getItem('checkoutState')) || "[]";
    const data = JSON.parse(localStorage.getItem('checkoutState') || '[]');
    ...
    if (Array.isArray(data) && data.length > 0) {
      data.forEach(category => {
        if (Array.isArray(category.cart_dtls)) {
          totalServices += category.cart_dtls.length;
        }
      });
    }
  
    setCartCount(totalServices); 
    // console.log("Total services selected:", totalServices);
  }, []);
    ...
    <>
      ...
      <Drawer anchor="left" open={drawerOpen} onClose={toggleDrawer(false)} >
        ...
        >
          <List>
            <ListItem>
              <img src={MrServiceLogo} alt="Mr Service Expert" />
            </ListItem>
