...
const handleTipSelection=(amount)=>{
    if(amount){
        // here will the logic of api will be done
        // console.log(amount+finalTotal);
        // toast.error(amount+"as a tip has been added")
        toast.info("Soon this feature comming stay tuned")
    }
    // if(amount===50){
    //     // here will the logic of api will be done
    //     console.log(amount+finalTotal);
                    ...
                    <div className="order-summary">
                        <h3>Order Summary</h3>
                            <div className="checkOutOrder">
                                <div>
                                    {cartDataArray?.map((service) => (
                                        <div key={service.leadtype_id} className="checkout-item flex-col service-card2 flex items-start">

                                           <p>{service.leadtype_name}</p>
                                         {service.cart_dtls?.map((items,index)=>(
                                             <div className="flex items-center gap-1.5 justify-center cartItemServices" key={items.service_id}>
                                            
                                             <div className="problemIcon">
                                                   <img src={items.image} alt={items.service_name} />
                                               </div>
                                               <div>
                                                   <p className="mb-0">{items.service_name}</p>
                                                  
                                                   <p className="text-xs text-gray-300" dangerouslySetInnerHTML={{ __html: items.description }}></p>
                                               ...
                                               <div className="flex items-center flex-col">
                                                   ...
                                                   <div>
                                                       <p className="text-xs text-gray-700 mb-1"> ₹{items.total_price}</p>
                                                       {/* <p className="text-xs text-gray-700"> ₹{items.price} x {items.quantity || 1} == {items.total_price}</p> */}
                                                   </div>
                                                   <div className="quantity-control">
                                                       <button className="IncrementDcrementBtn" onClick={() => onDecrement(items.service_id, 'delete', items.quantity)}>
                                                           -
                                                       </button>
                                                       <span>{items.quantity || 1}</span>
                                                       <button className="IncrementDcrementBtn" onClick={() => onIncrement(items.service_id, 'add', items.quantity)}>
                                                           +
                                                       </button>
                                                   </div>
                                               </div>
                           ...
                           <div className=" flex flex-col ">
                            <p className="mb-1.5 text-xl avaailHead text-purple-800"> <b>For Availing Services Go To Services Section</b></p>
                            ...
                            <Link to={'/service/ro-service'}>  <button className="go-back-btn w-full ">service </button></Link>
                            </div>
                           </div>
