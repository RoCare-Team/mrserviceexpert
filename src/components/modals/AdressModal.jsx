...
function AddressModal({ onAddressSelected, addressOpen, setAddressOpen }) {
  ...
  console.log(RecentAddress.address);
  // useEffect(() => {
  //   if (addressOpen) {
  //     try {
  //       const recentAddressData = localStorage.getItem('RecentAddress');
  //       if (recentAddressData) {
  //         const parsedData = JSON.parse(recentAddressData);
  //         console.log("Loaded address data:", parsedData); // Debug log
  //         setPreviousAddresses(parsedData);
  //       } else {
  //         console.log("No RecentAddress found in localStorage");
  //       }
  //     } catch (error) {
  ...
  //   }
  // }, [addressOpen]);
// console.log(RecentAddress.id +' the saved address id');
...
// console.log(savedAddress);
  ...
  const handleSelectAddress = (address,address_id) => {
    // console.log(address_id+"the id of address");
          ...
          <DialogContent sx={{ mt: 2 }}>
            ...
            <Box className="recentLocation">
              ...
              <Grid container spacing={2}>
                ...
                {RecentAddress?.address?.map((city, index) => {
                  return (
                 ...
                 {/* {previousAddresses && Array.isArray(previousAddresses) && previousAddresses.length > 0 ? (
  previousAddresses.map((city, index) => (
                    <Grid item xs={12} md={12} lg={12} key={index}>
                      <Paper
                        elevation={0}
                        variant="outlined"
                      ...
                      >
                        ...
                        >
                          {city.flat_no}, {city.landmark}, {city.area}, {city.state}, {city.city}, {city.pincode}
                        </Typography>
                        <Typography 
                          component="span" 
                          sx={{
                  ...
                  <Grid item xs={12}>
                    <Typography variant="body1" textAlign="center" color="text.secondary">
                      No addresses found. Add a new address to see it here.
                    </Typography>
                  </Grid>
                )} */}
