...
const TimeSlotModal = ({ onTimeSlotSelected, onClose, open }) => {
  ...
  const handleDateSelect = async (date) => {
    ...
    if (data && data.all_time_slots) {
      localStorage.setItem("time_slot", JSON.stringify(data.all_time_slots));
    }

    else {
      localStorage.setItem("time_slot", JSON.stringify([]));
    }
    // if (data && data.all_time_slots && Array.isArray(data.all_time_slots)) {
    //   setTimeSlots(data.all_time_slots);
    //   localStorage.setItem("time_slot", JSON.stringify(data.all_time_slots));
    // } else {
    //   // Handle case when no slots or invalid data
    //   setTimeSlots([]);
    //   localStorage.setItem("time_slot", JSON.stringify([]));
    // }
            ...
            <div className="h-40 overflow-y-auto">
              <div className="grid grid-cols-3 gap-2 p-3">
                {timeslot?.length > 0 ? (
                    ...
                    timeslot.map((time) => (
                      <div key={time.id}>
                        ...
                        >
                          ...
                          <span className={`text-xs ${selectedTime === time.slot_name ? "text-orange-500 extraCharge" : "text-orange-500 extraCharge"} ml-1`}>{time.slot_name}</span>
                        </button>
                      </div>
                    ))
                  ...
                  <p>No more time slot available. Go to next days for booking.</p>
                </div>
                )}
